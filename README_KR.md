...
- **지난 일정 흐리게 (딤 처리)**: 이미 종료된 지난 일정은 불투명도와 채도를 자동으로 낮추어(35%) 오늘 남은 일정과 명확하게 구분하며, 마우스 호버 시 즉시 100% 본래 색상으로 복원
...
- **일정 시작 전 알림 (브리딩 펄스)**: 다가오는 회의 시작 5분, 10분, 15분, 20분 전(선택 가능)에 바가 3.0초간 부드럽게 숨 쉬듯 차오르는 펄스(8px 도톰한 확장 및 14px 네온 오라)로 소리나 팝업 없이 사전 인지 지원
- **일정 시작 접점 효과 (1.0초 16프레임 실키 스무스 4종 테마)**:
  - `체리 블라섬`: 핑크빛 벚꽃 잎들이 폭죽처럼 팡 터지며 3D 펄럭임으로 흩날림
  - `네온 썬더`: 네온 시안과 화이트 벼락 줄기가 화면 안쪽으로 시원하게 방전
  - `메이플 리프`: 큼직한 5~7갈래 불가사리형 단풍잎(진홍/주황/황금)이 가을바람에 알록달록 흩날림
  - `스노우 플러리`: 6각 대칭 정밀 눈결정과 영롱한 네온 시안 블리자드 회오리 비산
...
- **표시 및 효과**: 일정 카드 스타일/테마/투명도, 시간 표시자 모양/색상, 일정 시작 전 알림(스위치, 시간 피커, 미리보기), 일정 시작 효과(스위치, 4종 사계절 효과 피커, 미리보기), 정각 알림 체크박스, 펫 선택(기본 백호/삼색이/진도백구), 숨김 동작, 커스텀 펫 관리
- **시간 및 캘린더**: 24시간 모드, 업무 시작/종료 시각, 표시할 캘린더 선택 및 캘린더별 색상 지정, 시스템 설정 1클릭 딥링크
- **일반**: 설정 언어 (시스템 기본/한국어/영어), 로그인 시 자동 실행, 다중 모니터 표시, 화상회의·녹화·가상화면에 표시 안 함, 전체 화면 시 숨김, 앱 정보
